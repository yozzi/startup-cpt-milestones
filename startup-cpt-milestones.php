...
function startup_cpt_milestones_updater() {
...
//add_action( 'init', 'startup_cpt_milestones_updater' );

//CPT
function startup_cpt_milestones() {
	$labels = array(
...
add_action( 'init', 'startup_cpt_milestones', 0 );

//Flusher les permalink à l'activation du plugin pour qu'ils fonctionnent sans mise à jour manuelle
function startup_cpt_milestones_rewrite_flush() {
    startup_cpt_milestones();
    flush_rewrite_rules();
}

register_activation_hook( __FILE__, 'startup_cpt_milestones_rewrite_flush' );

// Capabilities

function startup_cpt_milestones_caps() {
...
register_activation_hook( __FILE__, 'startup_cpt_milestones_caps' );
...
if ( !function_exists( 'cmb2_detection' ) ) {
    ...
    function cmb2_notice() {
        if ( current_user_can( 'activate_plugins' ) ) {
            echo '<div class="error message"><p>' . __( 'CMB2 plugin or StartUp Reloaded theme must be active to use custom metaboxes.', 'startup-cpt-milestones' ) . '</p></div>';
        }
    }
...
function startup_cpt_milestones_meta() {
    require ABSPATH . 'wp-content/plugins/startup-cpt-milestones/inc/font-awesome.php';
    
	// Start with an underscore to hide fields from custom fields list
	$prefix = '_startup_cpt_milestones_';
...
add_action( 'cmb2_admin_init', 'startup_cpt_milestones_meta' );

// Shortcode
function startup_cpt_milestones_shortcode( $atts ) {
...
add_shortcode( 'milestones', 'startup_cpt_milestones_shortcode' );
...
if ( !function_exists( 'shortcode_ui_detection' ) ) {
    ...
    function shortcode_ui_notice() {
        if ( current_user_can( 'activate_plugins' ) ) {
            echo '<div class="error message"><p>' . __( 'Shortcake plugin must be active to use fast shortcodes.', 'startup-cpt-milestones' ) . '</p></div>';
        }
    }
...
function startup_cpt_milestones_shortcode_ui() {

    shortcode_ui_register_for_shortcode(
        'milestones',
        array(
            'label' => esc_html__( 'Milestones', 'startup-cpt-milestones' ),
